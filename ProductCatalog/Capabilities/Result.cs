...
namespace ProductCatalog.Capabilities;
...
public sealed class Failed<TFailure> : Result<TFailure, IReadOnlyList<Failure>>
{
    ...
    public static Result<TFailure, IReadOnlyList<Failure>> FailedFor(Failure failure) =>
        new Result<TFailure, IReadOnlyList<Failure>>
        {
            IsSucceded = false,
            Failed = new ImmutableList<Failure>(failure)
        };
}